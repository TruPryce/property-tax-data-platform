...
if [ "$IMAGE_PROFILE_ID" != "plan.read-only.v1" ] || [ "$IMAGE_PROFILE_SHA" != "$EXPECTED_PROFILE_SHA" ] || \
   [ "$IMAGE_PROVIDER" != "$PROVIDER" ] || [ "$IMAGE_CODEX_VERSION" != "$EXPECTED_CODEX_VERSION" ]; then
...
python3 - "$RUN_DIR/container.provenance.json" "$IMAGE" "$IMAGE_ID" "$PROVIDER" "$CREDENTIAL" "$EXPECTED_PROFILE_SHA" "$EXPECTED_CODEX_VERSION" <<'PY'
import json
import pathlib
import sys

path, image, image_id, provider, credential, profile_sha, codex_version = sys.argv[1:]
document = {
    "contract_version": 1,
