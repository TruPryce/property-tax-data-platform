# release-resource-limits Specification

## Purpose
Define how the bounded release boundary's memory is measured and bounded: a peak-RSS probe whose
every sample names its source and whose cgroup path is resolved rather than assumed, a per-task
guard that rejects at its limit and covers the process it runs in, and an acceptance benchmark that
brackets its own attribution, refuses figures it cannot compare, and must calibrate its sensitivity
before a verdict counts. It supplies mandatory evidence for issue #43 D5 and not exhaustive proof of
it: what measurement cannot reach is recorded as unproven rather than approved.
