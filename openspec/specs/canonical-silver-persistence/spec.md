# canonical-silver-persistence Specification

## Purpose
Define how the promoted canonical appraisal domain is persisted, in a schema that sits beside
adapter-grain evidence rather than replacing it. A canonical release that either rests on the four
components its source supplied or does not exist, so a county-native label no contract has resolved
yields no identity rather than a derived one; a county that the version-controlled registry does not
describe being unpersistable rather than merely wrong; and a load that names one release, one
processing run, and the one artifact that run actually read, so no record can claim bytes its run
never opened. Bounded provenance as the single lineage authority, pinned to its load's release,
artifact, and county, with parent agreement at release grain — a child from a second artifact of one
release attaches, and a child crossing releases does not.

An account identity that is county-qualified, and a snapshot whose grain is an access path rather
than a constraint, because one account observed twice in one release is two rows that must both
survive. Every parented record one-to-many because nothing established otherwise, with no uniqueness
composed of anything a county said. A taxable value that cannot exist without its taxing unit, owner
allocations with no mechanism anywhere to assemble an account total, closed vocabularies that a
migration must be reviewed to widen, and the six carried source-native labels left open. Exact
decimals with no floating point and no invented precision, and geometry carried opaquely with no
geospatial dependency.

What the schema refuses is the point, and where it cannot refuse it says so: timezone-awareness is
the writing boundary's obligation because PostgreSQL accepts a value with no offset. Canonical
representation confers no publication permission, the reading role is denied the schema outright, and
the loading role may insert and select but never update or delete — so a retry that would overwrite
divergent evidence fails rather than succeeding quietly. Migrations are forward-only, refuse to
reapply or run out of order, and leave a database already carrying earlier ones structurally
identical to one built from empty.
## Requirements
