# canonical-appraisal-records Specification

## Purpose
Define the canonical appraisal record vocabulary that sits between county adapters and durable
storage, at the grain a county roll actually has. An account identity that is county-qualified and
distinct from any one release's observation of it; a snapshot whose grain is that identity and its
release, with provenance as the sole authority for which release that is; owner observations that
manufacture no person identity, and associations carrying the ownership percentages and allocations
that must never be rolled up into an account total; values keeping market, appraised, and assessed
apart from a taxable figure that cannot exist without the taxing unit it applies to; exemptions,
taxing units, land, and improvements at their own grain with county labels preserved rather than
canonicalized; and geometry as an enrichment carried opaquely, with its coordinate reference
required and no geospatial dependency.

Every record type is classified as a stable identity, a release-scoped snapshot, a child
observation, an association, or an enrichment, and that classification is published as data so a
storage layer reads it rather than inferring it from a primary key. What the shapes make
unrepresentable is the point: a property-wide taxable value, an account total assembled from owner
allocations, a negative measured area, a canonical value with no accepted equivalence behind it,
and a record carrying permission to publish what it represents.
## Requirements
