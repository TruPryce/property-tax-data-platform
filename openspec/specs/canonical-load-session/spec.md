# canonical-load-session Specification

## Purpose
Define one logical release load as a closed state machine: six state components over a fixed
context, three operations, every precondition named by an identifier, and one success and one
failure transition each. The transition table below is the authority for this capability. A
transition it does not define does not exist, and any text elsewhere that disagrees with it is a
defect in that text.

The table is here, in the spec, rather than in the design that argued for it, because this is what
survives promotion. It is also why the capability exists separately at all: the canonical load is a
stateful transactional protocol, and specifying one as prose spread across a spec, a design, a
decision list and a task list produced ten review rounds in which every correction was locally
right, changed the state space, and exposed the next interaction.

Two properties are worth reading before the rest. **Every failure row is empty across every
component** — one rule a reader checks by looking, rather than several a reader must assemble. And
account scope is `I1`, a *derived invariant* rather than a precondition: an earlier draft stated it
as a check with no reachable counterexample, which an implementation could not honour and a suite
could only appear to prove.

The boundary decides no key over observed values. Records the canonical model admits are not
discarded, merged, or rejected on the basis of what they contain, and no precondition here may be
read as a deduplication rule.
...
**`I1`, account scope, SHALL be a derived invariant and not a precondition.** Every handle in
`S4` SHALL belong to the account named by `S2`, at every point between operations, and a delta
SHALL therefore never be able to name a handle of any other account. This is derived rather than
checked: `W9` confines every retained handle to the account the batch carries onward, the success
transition releases the rest when their account completes, and `W7` confines a release to what is
live — so `S4` only ever holds the open account's handles, and a caller attempting otherwise is
refused by `W9` before any account-scope check could see it.

An earlier draft stated this as a write precondition, `W8`, and required refusals for it. That was
wrong in a way worth recording: the rule had **no reachable counterexample**, so an implementation
could not produce the refusal the specification demanded, and a suite could only appear to prove it
by triggering `W9` and calling it something else. The identifier `W8` is retired rather than
reused, so a reference to it in the review ledger resolves to this paragraph. What replaces it is
an obligation with teeth in the other direction: an implementation SHALL hold `I1` after every
operation, accepted or refused, and the falsification suite SHALL assert it there rather than
assert a refusal.

`W11` SHALL be decided against the accounts **open in the batch**, never against touched ones, for
the same reason `I1` holds: a delta may not be its own permission. It SHALL be the session's and not
...
`B1`–`B10` for the batch and `W1`–`W7` and `W9`–`W12` for the session, `W8` having been retired in favour of the derived invariant `I1`.
...
#### Scenario: Account scope holds as an invariant rather than a refusal
- **WHEN** a session is examined between operations, after an accepted batch and after a refused one alike
- **THEN** every handle in `S4` belongs to the account `S2` names, which is `I1`, and no batch can name another account's handle because none is there to name

#### Scenario: A batch tries to make an account its own by naming it in a delta
- **WHEN** a batch carries no entry and no adoption of an account, does not inherit it as `S2`, and names one of its handles in a delta
- **THEN** it is refused by `W9` when the handle is retained and by `W7` when it is released, because `I1` leaves no handle of another account for a delta to name, and `I1` still holds afterwards
...
An `AccountSnapshotRef` SHALL wrap a value that is **immutable and hashable**, and SHALL refuse
one that is not at construction. Opacity is not permission to carry anything: an implementation
indexes candidates by these locators, a caller may put them in a set, and a frozen wrapper around a
mutable payload is not frozen — its equality changes after construction and its hash raises. It
SHALL support equality and SHALL carry no ordering.
...
#### Scenario: A locator is offered a mutable or unhashable value
- **WHEN** an `AccountSnapshotRef` is constructed over a list, a dict, or any other value that cannot be hashed
- **THEN** it is refused at construction, rather than constructing and failing later when something puts it in a set or uses it as a key

#### Scenario: A locator is used as a key and compared
- **WHEN** locators are compared and used to index what they locate
- **THEN** equal locators are equal and hash alike, and no locator can be ordered against another
...
both carried by `S0`. A pairing SHALL be **complete** once any completion of it has succeeded,
whatever branch that completion took: a rejected run recorded its outcome and persisted no records,
and it is no less complete for that. Completion SHALL NOT be inferred from the presence of
canonical records, because the rejected branch persists none and a pairing judged by rows alone
would be retried forever — each retry rewriting the outcome the first one recorded, and a retry
carrying an accepted disposition manufacturing a canonical load for a run whose durable outcome
says it was rejected.

Completing a load for a pairing that has already completed SHALL persist
...
#### Scenario: A rejected completion is itself retried
- **WHEN** a pairing completes with a rejected outcome, and a second session for that same release and run completes
- **THEN** the second reports that the load was already complete and persists nothing, the first completion having completed the pairing even though it persisted no records

#### Scenario: A retry of a rejected pairing carries an accepted outcome
- **WHEN** a pairing completed with a rejected outcome and a later session for it carries an accepted one
- **THEN** it still reports already complete and persists no canonical records, because a pairing judged by its rows rather than by its completion would manufacture a load for a run whose durable outcome was rejected
