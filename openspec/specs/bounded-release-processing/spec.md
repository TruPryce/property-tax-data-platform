# bounded-release-processing Specification

## Purpose
Define the county-neutral boundary one logical release passes through: a single-pass reader
carrying release identity, row envelopes and row rejections, a caller-supplied atomic stage whose
records stay invisible until exactly one commit, the ordered lifecycle joining them, a closed
diagnostic vocabulary with a bounded outcome and a separate non-fatal notice channel, a
deterministic progress contract, and the resource-guard seam whose limits and measurement method
