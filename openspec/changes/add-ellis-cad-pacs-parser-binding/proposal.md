...
**Provenance.** The Ellis source contract in `bootstrap-six-county-appraisal-platform` and Issue #21 are the authoritative input. D1 through D4 are proposed by this change to close gaps that input leaves open — it fixes the obligations but states no fingerprint comparison mechanism, package signature, approved label value, or lexical bounds — and no prior maintainer selection is claimed for any of them. Merging this change is what accepts them.
