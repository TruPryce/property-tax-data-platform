...
The workspace is created from the trusted base with hooks, credential helpers, fsmonitor, SSH, Docker, Tailscale, host-home mounts, and production credentials disabled. A versioned command registry permits only exact repository-declared commands with bounded time/output, offline network by default, and phase/path policy. A separate path policy rejects writes outside the workspace and disallows workflows, CODEOWNERS, policies, providers, credentials, infrastructure, data archives, `.env`, `.git`, and other sensitive roots unless an accepted higher-risk flag explicitly permits them. No model output can expand either policy.
