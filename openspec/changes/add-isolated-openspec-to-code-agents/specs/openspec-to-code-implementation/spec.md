...
The implementation image SHALL be built from the same resolved request the runner consumes, so provider, model reference, reasoning effort, Codex CLI version, and profile digest cannot differ between the image producer and the runtime consumer. The trusted workflow MUST NOT state any identity value as its own literal. The image builder MUST refuse to build when any identity dimension is absent rather than supplying a default, and the runtime identity comparison MUST remain a single conjunction that refuses the run on any one mismatch.
