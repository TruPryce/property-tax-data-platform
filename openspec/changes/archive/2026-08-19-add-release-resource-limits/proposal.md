...
- **D42** *(amendment, added during implementation)*: the absolute check is **bracketed** rather than attributed by inference. The benchmark reads the cgroup peak before its measurements and after them: an initial reading at or above the limit means the cgroup arrived contaminated and no figure from it is attributable, so the verdict is indeterminate; initial below and final below is a pass; initial below and final at or above is a failure — a **conservative** verdict rather than an attributive one, since isolation is checked at the end rather than continuously and a transient neighbour would fail the run for memory it did not allocate. Failing closed on a figure that may not be ours is the safe direction to be wrong in.
  This replaces a root-process-age rule that implementation added and review rejected. Age appears to establish that a cgroup was created for the run and does not: a fresh shell with a few seconds of prior work passes it. Bracketing needs no claim about the cgroup's history at all, which is why it holds where age does not.
  Isolation verification is correspondingly narrowed to what it can actually establish, and strengthened where it was unsound. It requires a **complete** membership map — a pid whose parent cannot be read invalidates the map rather than being dropped, since dropping one is how a foreign tree becomes a clean-looking single one — and requires **this process to be a member**, because one tidy tree that is not ours says nothing about the figure being read.
- **D43** *(amendment, added during implementation)*: an unreadable or malformed `/proc/self/cgroup` makes the cgroup source **unavailable**, and never resolves to the mount root. Falling back to the root would read some other cgroup's figure under the name of this one — the exact failure the resolution rule exists to prevent, reached by the error path instead of the happy path. A caller naming `cgroup_v2` gets `PeakRssSourceUnavailable`; a caller naming nothing gets the documented `rusage` fallback, which is the only case a fallback was ever for.
