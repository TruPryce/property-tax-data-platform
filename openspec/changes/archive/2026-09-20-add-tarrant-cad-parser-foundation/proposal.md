...
Parsing, validation, diagnostics, fixtures, tests, and documentation are implementable now. That slice is a validator: it returns one bounded `TarrantValidationReport` of counts, diagnostics, fingerprint, and observed headers, and no field values. Both typed-record slices — the frozen `TarrantCertifiedSourceRecord` and the shared conversion — wait on Issue #43, because the approved native record holds shared `SourceNativeValue` entries and no county-local substitute may be written in the meantime.
...
- **D5** (resolved_for_draft, requires human merge): Keep Tarrant county-native parsing, physical records, and diagnostics Tarrant-specific; create no local vendor-neutral output, provenance, or native-value family; consume `SourceNativeValue`, `SourceProvenance`, and `AppraisalSourceRecord` from the accepted Issue #43 shared adapter contracts; and block any implementation task requiring those contracts until the corresponding #43 implementation has landed. Supplied by the maintainer on issue #19 as the dependency boundary.
- **D6** (proposed by this change, requires human merge): Accept dates matching `^(0?[1-9]|1[0-2])/(0?[1-9]|[12][0-9]|3[01])/([0-9]{4})$` — slash-separated, one- or two-digit month and day, four-digit year — additionally requiring calendar validity and the 1900–2100 range, preserving the original lexical text, and rejecting every other form with `invalid_source_date`. D2 fixed the components, validity, and range but named no separator; this supplies it.
- **D7** (proposed by this change, requires human merge): Bound the caller-supplied release identity. `release_identifier` and `source_member_name` are each 1 through 128 characters drawn from `[A-Za-z0-9._-]` and may not begin with `.` or `-`; `expected_source_year` is an `int` from 1900 through 2100. A violation raises `ValueError` before the member is read and produces no report and no diagnostic. D4 required bounded logical identifiers without stating a bound; this supplies one that makes absolute, UNC, drive-qualified, and traversal paths unrepresentable rather than merely discouraged.

**Provenance.** D1 through D5 come from authoritative maintainer input on issue #19: D1 through D4 from the four-part decision package, and D5 from the dependency-boundary comment. D6 and D7 do not. They are proposed by this change to close gaps the approved input left open — D2 fixed the date components but named no separator, and D4 required bounded identifiers but stated no bound — and no prior maintainer selection is claimed for either. Merging this change is what accepts them. Two further foundation choices are stated in the capability spec for the same reason and on the same footing: the exact fingerprint serialization, which D1 left unstated while requiring determinism, and the `unsupported_layout` trigger, which D4 named without assigning conditions.
