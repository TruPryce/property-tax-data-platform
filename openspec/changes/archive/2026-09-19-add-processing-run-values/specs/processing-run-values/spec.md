...
<!-- Owns: `ProcessingRunRef`, `ReleaseDisposition`,
     `ReleaseDiagnosticRecord`, `ReleaseNoticeRecord`,
     `ReleaseProcessingOutcome` with its evidence seal, and the application's
     own copy of the boundary contract version.
     Cites: `application-port-boundary` for the opaque-locator rule; the
     accepted `bounded-release-processing` spec for the closed diagnostic
     vocabulary and the notice grammar; the accepted
     `canonical-silver-persistence` spec for the outcome, diagnostic and notice
     records these values must be recordable as.
     Deliberately does NOT own the run lifecycle — starting, holding, resuming
     and finishing a run, and the repository that does it, stay with the
     `processing-run` capability in `add-application-port-boundary`, which
     cites this one for the reference and the outcome it carries. -->
