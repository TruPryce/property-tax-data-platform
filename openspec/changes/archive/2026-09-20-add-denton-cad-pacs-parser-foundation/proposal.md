...
- **D4** (proposed by this change, requires human merge): A core appraisal child orphan blocks the release; a legal child orphan warns without blocking. An undocumented trailing region **blocks**, because Issue #20 requires unknown trailing bytes to fail closed; its structural fingerprint is retained and its content discarded, so the rejection carries evidence without carrying data.
- **D5** (proposed by this change, requires human merge): Child lexical bounds are `child_sequence` one to four ASCII digits and required, and `child_value` the same monetary grammar as a property monetary field — `[0-9]+(?:\.[0-9]{1,2})?` bounded 0 through `10**26 - 1` — optional, with empty text after trimming as the only null. These are decided here rather than left to task 6.3, because an implementer choosing them would be making a source-contract decision under the guise of writing code.

**Provenance.** The Denton source contract in `bootstrap-six-county-appraisal-platform` and Issue #20 are the authoritative input. D1 through D5 are proposed by this change to close gaps that input leaves open — it fixes the obligations but states no positions grammar, fingerprint serialization, property or child lexical bounds, or orphan disposition — and no prior maintainer selection is claimed for any of them. Merging this change is what accepts them.

## Unresolved decisions

- Issue #43 has not supplied the accepted and implemented `SourceNativeValue`, `SourceProvenance`, `AppraisalSourceRecord`, bounded release-processing, and streaming contracts. Typed Denton and vendor-neutral record output, and typed child records, are therefore deferred to tasks 6.1, 6.2, and 6.3, which are recorded unchecked rather than omitted: Issue #20 is not complete until all three land. This change adds no county-local substitute while waiting, and tasks 1.1 through 5.1 are runnable without it.
