...
Selection is stable and bounded: approved repository roots only, normalized paths, symlink confinement, regular files, deterministic ordering, per-file and aggregate byte/file ceilings, explicit truncation metadata, and SHA-256 hashes. Issue comments are deduplicated and selected in a deterministic newest-first window of 16 entries; the triggering command comment is retained when it would otherwise fall outside that window. The packet records issue facts as untrusted source material, never as instructions. A manifest records every included source and every excluded candidate with a bounded category and reason code; packet and manifest hashes are required in the runner request.
