...
- **D2** (proposed by this change, requires human merge): The layout fingerprint is the lowercase SHA-256 of a canonical JSON document with exactly `component_contract_version`, `layout_id`, `layout_version`, `field_count`, and `fields`, serialized with keys sorted by code point, the separators `,` and `:` and no other whitespace, literal non-ASCII, and UTF-8 bytes. It is versioned separately from any export-header version.
