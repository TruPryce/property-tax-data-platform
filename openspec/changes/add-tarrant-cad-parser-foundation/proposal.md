...
- **D5** (resolved_for_draft, requires human merge): Accept dates matching `^(0?[1-9]|1[0-2])/(0?[1-9]|[12][0-9]|3[01])/([0-9]{4})$` — slash-separated, one- or two-digit month and day, four-digit year — additionally requiring calendar validity and the 1900–2100 range, preserving the original lexical text, and rejecting every other form with `invalid_source_date`. D2 fixed the components, validity, and range but named no separator; this supplies it.
- **D6** (resolved_for_draft, requires human merge): Bound the caller-supplied release identity. `release_identifier` and `source_member_name` are each 1 through 128 characters drawn from `[A-Za-z0-9._-]` and may not begin with `.` or `-`; `expected_source_year` is an `int` from 1900 through 2100. A violation raises `ValueError` before the member is read and produces no report and no diagnostic. D4 required bounded logical identifiers without stating a bound; this supplies one that makes absolute, UNC, drive-qualified, and traversal paths unrepresentable rather than merely discouraged.

**Provenance of D5 and D6.** Neither was supplied by an issue comment. D5 was chosen by the maintainer during review of this pull request, and D6 was proposed during that review to close a gap D4 left open. Both are recorded here so that merging approves them explicitly rather than silently. Two further foundation choices are stated in the capability spec for the same reason: the exact fingerprint serialization, which D1 left unstated while requiring determinism, and the `unsupported_layout` trigger, which D4 named without assigning conditions.
