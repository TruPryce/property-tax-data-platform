...
3. **Lexical layer** — the approved division, year, account, identifier, text, monetary, and D5 date grammars; empty-text-only nulls; release-wide account uniqueness; and release-level atomic rejection with bounded, deterministically truncated diagnostics.
...
- **D5** (resolved_for_draft, requires human merge): The date separator D2 left unstated is a solidus, with one- or two-digit month and day and a four-digit year: `^(0?[1-9]|1[0-2])/(0?[1-9]|[12][0-9]|3[01])/([0-9]{4})$`. Calendar validity, the 1900–2100 range, and lexical preservation come from D2. Like the other four, this is a synthetic-foundation contract and not reproduced live evidence.
- **D6** (resolved_for_draft, requires human merge): `release_identifier` and `source_member_name` are 1-128 characters from `[A-Za-z0-9._-]`, not beginning with `.` or `-`; `expected_source_year` is an `int` in 1900-2100; a violation raises `ValueError` before the member is read. D4 required bounded identifiers without a bound, and an alphabet with no path separator makes a host-local location unrepresentable rather than discouraged.

D5 and D6 came from review of this pull request rather than from an issue comment, so they are listed explicitly and merging is what accepts them.
