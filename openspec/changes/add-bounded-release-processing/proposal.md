...
- **D20** (proposed by this change): `layout_fingerprint` and `parser_contract_version` on the outcome are optional and are `None` when the failure preceded a successful preparation; `boundary_contract_version` is always populated because it is the boundary's own constant. A `source_open_failed` outcome has no reader to have produced a fingerprint, and a `layout_rejected` one may have failed before computing it. Declaring them always-present would force an implementation to invent a placeholder, and a fabricated fingerprint is worse than an absent one because a diagnostic could not tell the two apart.
