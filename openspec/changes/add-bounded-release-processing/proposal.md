...
- **D21** (proposed by this change): `PreparedReader` declares `prepare() -> PreparedRelease` as a named operation distinct from `__enter__`. The diagnostic table assigns `source_open_failed` to entry and `layout_rejected` to preparation, and without a call to attribute each failure to, an implementation could only guess which applied by inspecting where inside `__enter__` an exception arose. Naming the call is what makes both codes deterministically implementable.
- **D22** (proposed by this change): `ReleaseOutcome` has an exact frozen schema — a two-member `ReleaseDisposition` enumeration, a required boundary contract version, two optional prepared fields, a bounded diagnostic tuple with its preserved total and derived truncation flag, and four named non-negative counts — with stated invariants tying the counts to the disposition. An underspecified outcome is one every implementation renders differently, which is the opposite of a contract.

- **D23** (proposed by this change): every callable on the boundary declares an exact signature, listed in one table in the capability spec — the reader's `prepare` and `__iter__`, the stage's four methods, the callback's `__call__`, the guard's `check`, and `process_release` itself, which is keyword-only so a caller cannot transpose the reader and the stage. `write` takes a whole physical row's records together, because a row is the unit accepted or rejected and a split write could leave half a refused row staged. Both `__enter__` methods return the entered object itself, and both `__exit__` methods are annotated as returning `None` rather than `bool`, so non-suppression is carried by the signature instead of by prose an implementation could overlook — a suppressed failure is one the processor cannot map to a code. A `write` call is all-or-nothing: if it raises, none of that call's records are staged, because a stage holding a prefix of a row would make `staged_record_count` describe neither what was attempted nor what is present. D21 named one operation; without the rest, two conforming implementations could still be incompatible.
- **D25** (proposed by this change): disposition follows from the **failure diagnostics only**. Every code in the diagnostic vocabulary is a failure code, so a release is `accepted` exactly when it produced none. Leaving the two independent would permit an accepted outcome carrying `stage_commit_failed`, which asserts two contradictory things at once. Notices are excluded from this rule and are the subject of D26.
- **D26** (proposed by this change): the boundary carries a `ReleaseNotice` channel separate from diagnostics, reportable on `PreparedRelease` for a layout observation and on `SourceRowEnvelope` for a row one, bounded and truncated on the same terms, and never affecting the disposition. This is required by an accepted contract rather than convenient: the Dallas contract says unknown columns SHALL be **accepted** and reported with `extra_columns_present`, so with one channel a future Dallas reader would have to reject a valid layout or discard a required warning. The code is a county-supplied identifier of 1 through 64 characters matching `[a-z][a-z0-9_]*` and the boundary enumerates none, because notice vocabularies are county vocabularies closed by county contracts; the bound is what stops free text arriving where a code belongs.
- **D24** (proposed by this change): `BOUNDARY_CONTRACT_VERSION` and `PROGRESS_CONTRACT_VERSION` are both the integer `1`, declared as constants and asserted. A version field whose initial value is unpinned cannot tell a consumer which contract it is looking at, which is the only thing a version field is for.

**Provenance.** Issue #43's decision comments are the authoritative input. D1 through D8 restate them; where wording is condensed, the comment governs. D9 through D26 are this change's own and no prior maintainer selection is claimed for them. D14 adopts a bound the accepted Tarrant contract already sets rather than proposing a new one.
