...
| the stable boundary codes, which D5 gives as a minimum of eight | **this change**, at eleven |
...
- **D15** (proposed by this change): the conformance suite rejects **input-proportional** accumulation, not buffering. It drives a candidate reader from a guarded pull source, computes the lead as pulls minus records written, and requires the maximum lead to stay within a declared constant *and* to be identical across two releases of materially different length. A constant-size one-row buffer conforms, because its lead does not grow; a reader that materializes its member does not. An earlier draft required pulls at most equal to rows written, which would have failed a legitimate bounded buffer, and the draft before that depended on a generator that "would exhaust memory", which is resource behaviour this change defers.
...
- **D19** (proposed by this change): every operation that may fail completes before the commit. The reader is closed *before* committing, so no reader failure can occur after records become visible, and a stage's exit after a successful commit may not fail, which is what makes it safe to place last. No outcome may report rejection after a commit succeeded.

**Provenance.** Issue #43's decision comments are the authoritative input. D1 through D8 restate them; where wording is condensed, the comment governs. D9 through D19 are this change's own and no prior maintainer selection is claimed for them. D14 adopts a bound the accepted Tarrant contract already sets rather than proposing a new one.
