...
- **D25** (proposed by this change): disposition follows from the diagnostics. Every code in this vocabulary is a failure code, so a release is `accepted` exactly when it produced none and `rejected` when it produced at least one. Leaving the two independent would permit an accepted outcome carrying `stage_commit_failed`, which asserts two contradictory things at once.
...
**Provenance.** Issue #43's decision comments are the authoritative input. D1 through D8 restate them; where wording is condensed, the comment governs. D9 through D25 are this change's own and no prior maintainer selection is claimed for them. D14 adopts a bound the accepted Tarrant contract already sets rather than proposing a new one.
