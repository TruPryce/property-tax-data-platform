...
handle a batch has no other business with cannot be released by being released.

`W8` SHALL be an invariant rather than a reachable refusal, and this specification states that
rather than implying a counterexample that does not exist. `W9` confines every retained handle to
the account the batch carries onward, the success transition releases the rest when their account
completes, and `W7` confines a release to what is live — so `S4` only ever holds the open
account's handles and no batch can name another account's. A caller attempting it is refused by
`W9` first. `W8` SHALL therefore be proven as the invariant it guards: every live handle belongs
to the open account, before and after any refusal.
