...
| --- | --- | --- | --- |
| `canonical-load` | **ACCEPT** (round 1 at `51ef6bd`; its three reconciliation defects resolved at `1605458`, on the reviewer's recommended path — extract, accept, implement without a further architecture round) | 1 | `1605458` |

**Change status:** ACCEPTED and implemented. Tasks 1.1 through 3.1 are done on
this change's own branch, and the next review is of the implementation rather
than the plan.

One correction the implementation forced is recorded in
[`round-1.md`](round-1.md) under *Found while implementing*: `W8` was listed as
a refusal and has no reachable counterexample, so the spec, the matrix and the
suite now state and prove it as the invariant it guards. A rule that cannot be
falsified is the defect class this change exists to remove, so it is recorded
rather than quietly satisfied by a test that triggers a different rule.
