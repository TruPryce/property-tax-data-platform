...
- `W8` is proven as an **invariant**, not as a refusal, because writing the suite showed it has no
  reachable counterexample: `W9` confines a retained handle to the continuing account, completion
  sweeps the rest, and `W7` confines a release to `S4`, so `S4` only ever holds the open account's
  handles. The attempt lands on `W9`. What the suite asserts is that every live handle belongs to
  the open account, before and after a refusal — the property `W8` exists to guarantee, and the
  reason an account is open in a batch by what it **carries** and never by what its deltas name.
