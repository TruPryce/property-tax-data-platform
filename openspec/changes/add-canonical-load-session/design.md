...
IDs `B1`–`B10`, `W1`–`W7` and `W9`–`W12`, `C1`–`C2`, `A1`, `G1`, the derived invariant `I1`, and the transitions of each operation.
...
- Each of W1–W7 and W9–W12 refuses **on write**, with a session that has the history the check
  needs. There is no `W8`: it is retired, and `I1` replaces it as an invariant the suite asserts
  rather than a refusal it cannot produce.
- `I1` is asserted as an **invariant** after every operation, accepted or refused: every handle in
  `S4` belongs to the account `S2` names. It is not a refusal and the suite does not claim one —
  the attempt to name another account's handle is refused by `W9` when retained and `W7` when
  released, because `I1` leaves nothing of another account in `S4` for a delta to name.
...
- `W11` is falsified by a batch naming as continuing an account it neither **carries** nor
  ...
  against accounts open in the batch, never touched ones, for the same reason `I1` holds, and the
