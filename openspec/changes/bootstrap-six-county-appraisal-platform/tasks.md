...
- [ ] 3.5 Implement bounded batch parsing and PostgreSQL COPY-to-staging plus set-based idempotent merges — including the durable spill for an account whose parent observations outnumber what the loading session holds live, which the application port boundary states as a bound and deliberately does not solve, and lazy iterator access to the adoptable snapshots for one account and release, with the locator-and-snapshot agreement adoption verifies, whose count no accepted contract bounds
- [ ] 3.6 Add containerized object-store and PostgreSQL integration tests for retry, conflict, quarantine, and rollback behavior — including the oversized-parent case end to end: an account whose parent observations outnumber what the loading session holds live completes against a real database through 3.5's durable spill, and adoptable-snapshot candidates are drawn lazily rather than materialised, both of which the application port boundary states as bounds it does not itself close
