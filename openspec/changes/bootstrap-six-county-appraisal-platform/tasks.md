...
- [x] 2.2 Implement canonical account, owner, value, exemption, jurisdiction, land, improvement, and geometry record types
...
- [x] 3.2 Implement the Amazon S3 Bronze adapter behind the S3-compatible port and persist immutable release manifests
- [x] 3.3 Implement archive inspection and extraction limits for traversal, expansion, compression ratio, members, and media types
- [x] 3.4 Create PostgreSQL release-manifest, Silver canonical, diagnostic, quality-result, and publication metadata migrations
- [ ] 3.5 Implement bounded batch parsing and PostgreSQL COPY-to-staging plus set-based idempotent merges — choosing the session's stated maximum for what one batch may carry, entries and both correlation deltas together, and including durable maintenance of the logical live correlation set — the accumulation of the bounded retain and release deltas each batch carries, which the application port boundary deliberately leaves unbounded and names no mechanism for, which a released handle leaves rather than re-enters, and which rolls back with a refused batch so the durable live set and the highest handle yet introduced stay exactly as the last accepted batch left them — and lazy iterator access to the adoptable snapshots for one account and release, with the locator-and-snapshot agreement adoption verifies, whose count no accepted contract bounds
- [ ] 3.6 Add containerized object-store and PostgreSQL integration tests for retry, conflict, quarantine, and rollback behavior — including the oversized-parent case end to end: an account whose accumulated live correlation set exceeds memory completes against a real database through 3.5's durable maintenance of it, no single batch having enumerated that set, while a released handle stays refused, a batch refused against that real database leaves the durable live set and high-water mark unmoved so a corrected retry reusing its handles succeeds and contributes no staged row to the completed load, and adoptable-snapshot candidates are drawn lazily rather than materialised, both of which the application port boundary states as bounds it does not itself close
