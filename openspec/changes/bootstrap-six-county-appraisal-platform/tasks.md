...
- [ ] 3.5 Implement bounded batch parsing and PostgreSQL COPY-to-staging plus set-based idempotent merges — including the durable spill that keeps a large **declared** still-needed correlation set resolvable when it outgrows memory — the application port boundary states that bound and deliberately names no mechanism, and a handle the caller never declared is released by contract and is not what the spill rescues — and lazy iterator access to the adoptable snapshots for one account and release, with the locator-and-snapshot agreement adoption verifies, whose count no accepted contract bounds
- [ ] 3.6 Add containerized object-store and PostgreSQL integration tests for retry, conflict, quarantine, and rollback behavior — including the oversized-parent case end to end: an account declaring more still-needed correlation handles than fit in memory completes against a real database through 3.5's durable spill, while a handle never declared stays refused, and adoptable-snapshot candidates are drawn lazily rather than materialised, both of which the application port boundary states as bounds it does not itself close
