...
- [ ] 3.5 Implement bounded batch parsing and PostgreSQL COPY-to-staging plus set-based idempotent merges — implementing the canonical load session against the transition table in `openspec/changes/add-canonical-load-session/design.md` rather than any restatement of it: choosing `max_batch_entries`, durably maintaining `S4` when one continuing account's accumulation exceeds memory without extending it, honouring every failure row against the durable store including its staged rows, and offering adoptable-snapshot candidates through a lazy iterator whose count no accepted contract bounds
- [ ] 3.6 Add containerized object-store and PostgreSQL integration tests for retry, conflict, quarantine, and rollback behavior — including the canonical load session's transition table proved against a real database: the oversized-`S4` case end to end, an account whose accumulated live set exceeds memory completing with no single batch enumerating it, every failure row leaving `S1`-`S6` and the staged rows exactly as they were, a released handle staying refused, and candidates drawn lazily rather than materialised
