...
## The outcome the run carries is not this change's to define

`ReleaseProcessingOutcome`, its diagnostic and notice carriers, the disposition and the evidence
seal belong to the promoted `processing-run-values` capability, which states why the application
holds its own outcome rather than the adapters', why the mapping between them is total in both
directions, and why the seal is enforced in the value rather than at COMMIT.

They were this change's when it was written, and moved when implementing the canonical load session
showed that a capability citing them could not be built while they were still a plan. What stays
here is the *lifecycle*: `ProcessingRunRepository` starts a run, refuses one whose run is held,
resumes one whose holder is gone, and finishes it. The run this change starts carries an outcome it
does not define.
...
| `ProcessingRunRepository`, the active-run refusal, the abandoned-run resume | `processing-run` | `canonical-load-session`, `run-bound-quality-and-publication` |
| `ProcessingRunRef`, `ReleaseProcessingOutcome`, `ReleaseDiagnosticRecord`, `ReleaseNoticeRecord`, `ReleaseDisposition`, the evidence seal, the mirrored `BOUNDARY_CONTRACT_VERSION` | promoted `processing-run-values` | cited here and by `canonical-load-session`; defined by neither |
