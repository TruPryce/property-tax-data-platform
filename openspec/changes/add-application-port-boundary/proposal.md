...
- **D2a — A batch is closed over whole account groups.** Every canonical relation carries `snapshot_key`, so the graph roots at the account snapshot. A batch therefore carries complete account subtrees, and a child separated from its parent is refused. This is what lets an implementation resolve parent relationships inside one batch — without correlating across batches, without keying on observed values, and without a release-wide mapping that would grow with the release and undo the boundedness the session exists for.
- **D2b — The run has a producer.** `ProcessingRunRepository` starts a run and returns its reference. Without it the boundary names a run everywhere and creates one nowhere, and a use case would have to reach past the port into the database to obtain a value the database generates.
