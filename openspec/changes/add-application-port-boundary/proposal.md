...
Bootstrap 2.4 and 3.5 both need stable application ports to build against, and the application
package owns two today — `ArtifactSink` and `BronzeStore` — and nothing for runs, discovery,
quality, publication, or time.

This change adds those ports and corrects two retained values the code already contradicts: an
acquisition must be manifestable before its releases are known, and a manifest keyed by artifact
digest cannot hold two acquisitions of the same bytes.

Two capabilities that began here have since left, because implementing what cited them proved they
could not stay plans: the canonical load session, and the processing-run values. Both are promoted,
and this change cites them.
...
| `processing-run` | `ProcessingRunRepository` — the run lifecycle, and none of the run vocabulary, which the promoted `processing-run-values` capability owns | `application-port-boundary`; `source-registry-and-discovery` for the promoted identity; `acquisition-manifest-index` for `ManifestRef`; accepted `bounded-release-processing` for the diagnostic vocabulary |
...
Three decisions left this change with the values they decided. The run reference being an
application concept rather than a persistence key, the outcome being the application's own with a
total mapping, and the evidence seal living in the value are now the promoted
`processing-run-values` capability's `D1`, `D2` and `D3`. This change cites that capability and
neither restates nor revisits them.
...
- **D14 — Release-level source freshness reaches publication through the boundary, one instant per logical release.** `publication.publication.source_as_of` is what a consumer means by freshness, and nothing else in the boundary carries it: `ReleaseIdentity` has no time component and `ProcessingRunRef` is opaque. It is the instant the release's own `LogicalReleaseEvidence` carries, supplied when the attempt opens, so two releases drawn from one artifact each publish their own. It is never derived from canonical snapshots, because `0011` makes snapshot equality structural over every field *but* `source_as_of`, so one release can hold snapshots disagreeing about it. It is durable before publication needs it: the instant is recorded on the acquisition's **partition**, one partition being one logical release, and rides in the object-store manifest whose shape D18 is already versioning — so no migration and no relational column is added for it. That matters because discovery, acquisition, loading and publication are independently retryable: a publication resuming in a later process has no candidate to read, only the run and the manifest reference it was started from. Rediscovering the page is not the retrieval path, since the page is mutable and freshness that changes when you look again is not evidence.
