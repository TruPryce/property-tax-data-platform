...
**The instant SHALL be durable before publication needs it, and SHALL NOT exist only on the
candidate discovery returned.** Discovery, acquisition, loading, and publication are independently
retryable, so a publication resuming in a later process has no candidate to read: it has the run,
and through the run the manifest reference the run was started from. The instant SHALL therefore be
recorded with the acquisition's partition, where one partition is one logical release, and SHALL be
retrievable from that record for the release being published. Rediscovering the page SHALL NOT be
the retrieval path — the page is mutable, so a second read may yield a different instant or none,
and freshness that changes when you look again is not evidence of anything.

A retried publication SHALL therefore receive the same instant the first attempt would have, or the
same recorded absence.

#### Scenario: Publication resumes in a later process
- **WHEN** discovery, acquisition and loading completed in earlier processes and only publication remains
- **THEN** the attempt receives the release's instant from what the acquisition recorded, without a candidate in memory and without reading the source page again

#### Scenario: The page changes between discovery and a retried publication
- **WHEN** the source page is edited after discovery and the publication is retried afterwards
- **THEN** the attempt still receives the instant discovery established, because the durable record is what it reads and the page is not
