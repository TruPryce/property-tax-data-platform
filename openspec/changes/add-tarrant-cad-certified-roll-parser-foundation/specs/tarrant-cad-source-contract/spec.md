...
- **WHEN** the Tarrant parser validates the fixture layout
- **THEN** the layout is classified as unsupported
- **THEN** the approved bounded diagnostic is produced
- **THEN** no source record and no normalized record are emitted
...
- **WHEN** the parser processes the fixture as one release
- **THEN** the duplicate account produces the approved bounded diagnostic
- **THEN** division remains separate from account identity
- **THEN** no partially accepted release is returned
...
- **WHEN** the parser converts the row into a typed source record
- **THEN** the approved field-specific diagnostic and the logical row number are emitted
- **THEN** no floating-point conversion is performed
- **THEN** no typed record is emitted for the malformed release
...
- **WHEN** the adapter converts the Tarrant source record
- **THEN** the Total_Value fact remains source-native
- **THEN** no canonical market value is populated
- **THEN** no tax-collection interpretation is emitted
...
- **WHEN** the adapter creates its normalized output
- **THEN** no exemption observation and no current-roll classification are created
- **THEN** no jurisdiction-taxable observation and no companion relationship are created
- **THEN** no same-year replacement assertion is created
...
- **WHEN** the parser reports the invalid account
- **THEN** the diagnostic contains only its approved stable code and bounded location metadata
- **THEN** the account literal is absent from the diagnostic
- **THEN** the sentinel identity text, the mailing text, and the complete row are absent
...
- **WHEN** the adapter parses and converts the fixture
- **THEN** each record retains approved release, member, row, and layout evidence
- **THEN** no domain object is constructed and no network request is made
- **THEN** no database action, publication action, service call, or DAG action occurs
