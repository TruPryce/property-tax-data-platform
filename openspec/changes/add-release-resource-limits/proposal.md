...
| memory must not grow linearly with release row count | **shared**: shape by `bounded-release-processing`, budget by this change (D40) |
...
- **D40**: the benchmark is evidence for issue #43 D5's linearity requirement but is **not** a proof of it, and this change does not claim otherwise. Measurement resolution puts a floor under what any ratio can see: retention at or below three bytes per row moves `ru_maxrss` by 0.25 MiB or less across a fourfold row increase and is invisible at any threshold. What the benchmark establishes is that one task fits the per-task budget and that **record retention** — the failure mode this platform actually has, at hundreds of bytes per row — is caught with wide margin.
  The structural half of D5 is already discharged, and not by this change. The accepted `bounded-release-processing` capability requires a reader whose lead over consumption stays inside a fixed constant and is *identical* at 1,000 and 8,000 envelopes, and bounds every diagnostic and notice carrier at a hundred entries retained. Those are structural checks on shape rather than measurements of memory, so they do not have a resolution floor, and they are what forbids per-row retention. This change measures the budget; that capability constrains the shape; neither alone answers D5 and together they do.
