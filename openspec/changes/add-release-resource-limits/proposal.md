...
| memory must not grow linearly with release row count | **partly**: this change evidences it above the resolution floor and proposes narrowing D5 (D40, U1) |
...
- **D36**: linearity is tested as a **ratio between two release sizes**, not as an absolute, and what the ratio can establish is bounded by D40 — material record retention rather than all growth. The benchmark runs at 250,000 and at 1,000,000 rows and requires the peak to grow by less than a stated fraction of the fourfold row increase. An absolute threshold cannot distinguish a bounded implementation from a linear one that happened to fit.
- **D40**: the benchmark is **evidence** for issue #43 D5's linearity requirement and not a proof of it, and this change asks the maintainer to narrow D5 rather than claiming to have satisfied it. Measurement resolution puts a floor under any ratio: retention at or below three bytes per row moves `ru_maxrss` by 0.25 MiB or less across a fourfold row increase and is invisible at any threshold, and four bytes per row straddles it. What the benchmark does establish is that one task fits the per-task budget and that **material record retention** — hundreds of bytes per row, which is what this platform's readers would actually retain — is caught with wide margin.
  No existing check closes the remainder, and an earlier draft of this decision wrongly said one did. The accepted capability's reader-lead check measures pulls against envelopes consumed, so it detects **read-ahead**; a reader that yields an envelope and then keeps a compact summary of it leaves the lead unchanged and passes. Its hundred-entry caps bound **notices and diagnostics** and say nothing about any other retention. Neither forbids per-row retention in general, and citing them as if they did would have been the same overclaim this change has had to withdraw before.
...
- **U1 — narrowing issue #43 D5's linearity requirement.** D5 requires that memory not grow linearly with row count. This change can evidence that only above a measured resolution floor: retention of three bytes per row or fewer is invisible to `ru_maxrss` at the acceptance sizes, and four bytes straddles the threshold across repeats. No other accepted check closes the gap — the capability's reader-lead check detects read-ahead rather than retention, and its hundred-entry caps bound only notices and diagnostics.
  The proposal is therefore to narrow D5 to **material record retention**, the failure mode this platform actually has at hundreds of bytes per row, and to accept that sub-five-byte-per-row retention is unproven by measurement. That narrowing is a maintainer decision recorded in issue #43, not something this change may settle: a planning change cannot approve its own supersession of an accepted decision. **This item blocks acceptance of the change until a maintainer records it.**
