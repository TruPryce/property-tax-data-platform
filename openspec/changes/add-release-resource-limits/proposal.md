...
D5's 900 MiB supersedes the issue body's 1 GiB, because the scheduler's measured 419 MiB peak shares the same 4 GiB container: `(4096 - 419) / 4 ≈ 919`, and the target is set at 900.
...
- **D39**: the two checks use different sources, because they answer different questions. The **scaling ratio** uses `rusage`, the only per-process source: siblings share a cgroup whose peak is sticky and charges the whole subtree, so three subprocesses reading it report figures describing the subtree rather than the reader — 306.0, 449.5, and 454.2 MiB measured for siblings allocating 100, 200, and 400 MiB — and the resulting ratio tracks read timing rather than growth. The **900 MiB absolute** uses the cgroup, because issue #43 D5 makes the cgroup authoritative for the limit and it is the number the OOM killer acts on. Reporting a rusage figure against a cgroup-derived budget would compare quantities the change itself calls incomparable. The absolute check therefore requires the benchmark to run in a cgroup containing only that run.
