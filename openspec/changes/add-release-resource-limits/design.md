...
2. memory must not grow linearly with row count — a **shape**, and one this change can only partly evidence. Its structural half is discharged by the accepted `bounded-release-processing` capability, whose reader lead is bounded by a constant and identical across two fixture lengths and whose carriers retain at most a hundred entries; those are checks on shape and have no measurement floor. This change contributes the budget evidence, which does have one — see D40.
