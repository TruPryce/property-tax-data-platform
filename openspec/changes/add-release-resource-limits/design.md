...
- The scheduler container is capped at 4 GiB with `AIRFLOW__CORE__PARALLELISM=4`, and the scheduler's own measured peak is 419 MiB, measured on the deployment host. `(4096 − 419) / 4 ≈ 919 MiB` per task, against which the target is set at 900 MiB. `memory.peak` is monotonic, so that margin only narrows: the target stops fitting once the scheduler peak reaches 496 MiB.
