...
State stores repository/target identity, current run and idempotency key, command/profile/head, attempt, monotonic revision, workflow/check IDs, lifecycle state, lease, timestamps, disposition, and sanitized GitHub links. Rendering updates the existing comment rather than creating status spam. GitHub does not support conditional writes (`If-Match`/`412`) on the issue-comment update endpoint, so canonical mutation is serialized by a shared per-target job concurrency lane instead of an atomic API compare-and-swap. Inside that lane every intake, execution, and maintenance writer rereads the exact bot-owned comment, requires its complete validated state (including monotonic revision) to equal the expected predecessor, and then sends a plain comment update; a stale predecessor fails closed with `state_write_conflict`. The same publication mirrors the resulting lifecycle into an existing PR check only after the canonical comment update succeeds. Terminal run data is immutable; display-only reconciliation may refresh links or timestamps without rewriting prior artifacts.
...
- **The shared per-target state lane keeps only one pending job, so a burst can silently supersede a queued state transaction** -> This is accepted for v1: compare-before-write protects committed state from corruption, and a superseded control command (status/cancel) or terminal publish is recovered by a maintainer retry or the next status/maintenance reconciliation. Cancellation intent already persists once written, and a superseded terminal publish is caught by the live-lease requirement plus stale reclamation. Operators should retry a control command that shows no effect; a bounded user-visible superseded signal is deferred.
- **Maintenance runs in a repository-wide lane, not the per-target state lane, so it could race a late owner publish** -> Every canonical publication requires a live lease; once a lease expires the owner fails closed and only the `stale` reclamation path recovers the run. An expired lease therefore means no owner writer remains, so maintenance cannot lose a race with, or be overwritten by, a lapsed owner, and completed evidence is preserved without an atomic comment primitive.
