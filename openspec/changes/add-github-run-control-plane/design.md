...
State stores repository/target identity, current run and idempotency key, command/profile/head, attempt, monotonic revision, workflow/check IDs, lifecycle state, lease, timestamps, disposition, and sanitized GitHub links. Rendering updates the existing comment rather than creating status spam. GitHub does not support conditional writes (`If-Match`/`412`) on the issue-comment update endpoint, so canonical mutation is serialized by a shared per-target job concurrency lane instead of an atomic API compare-and-swap. Inside that lane every intake, execution, and status writer rereads the exact bot-owned comment, requires its complete validated state (including monotonic revision) to equal the expected predecessor, and then sends a plain comment update; scheduled maintenance is read-only discovery and never enters the write path. A stale predecessor fails closed with `state_write_conflict`. The same publication mirrors the resulting lifecycle into an existing PR check only after the canonical comment update succeeds. Terminal run data is immutable; display-only reconciliation may refresh links or timestamps without rewriting prior artifacts.
...
The canonical lease records owner workflow run/attempt, semantic key, command, target SHA, acquired/heartbeat/expiry timestamps, and a nonce. A shared per-target `countyforge-state-<repository-id>-<target-type>-<target-number>` job concurrency lane serializes canonical mutations, and a monotonic revision advanced exactly once per successful mutation provides application-level stale-state detection; each transition requires the expected revision. The state lane, the short control lane, and the target execution lane remain independent, so cancellation/status do not wait behind a provider job and only the small state transaction is serialized. A stale status, cancellation, claim, or terminal publication preserves a newer state or fails with `state_write_conflict` instead of replacing it; maintenance only reports candidates. Stage transitions update heartbeats. Check creation and check-ID publication failures convert an already-published queue to a retryable terminal failure. A queued run with no workflow owner or lease has a 30-minute preclaim deadline that status or a later command converts to `workflow_claim_timeout`. An authorized new command or `/countyforge status` can reclaim an expired leased run by transitioning the old run to `stale` inside the state lane; completed evidence is never replaced. Maintenance never writes or starts model work.
...
- **GitHub comment state is not a transactional database** -> Serialize canonical mutation in a shared per-target `countyforge-state-*` job concurrency lane (GitHub does not support `If-Match`/`412` comment writes), validate marker ownership/schema, reread-and-compare the monotonic revision before a plain update, and use leases plus idempotent recovery.
...
- **The shared per-target state lane keeps only one pending job, so a burst can silently supersede a queued state transaction** -> This is accepted for v1: compare-before-write protects committed state from corruption, and a superseded control command (status/cancel) or terminal publish is recovered by a maintainer retry or the next status/authorized-command reconciliation. Cancellation intent already persists once written, and a superseded terminal publish is caught by the live-lease requirement plus stale reclamation. Operators should retry a control command that shows no effect; a bounded user-visible superseded signal is deferred.
- **Maintenance runs in a repository-wide lane, not the per-target state lane, so it could race a late owner publish** -> Scheduled maintenance is audit-only in v1: it discovers and reports candidates but never writes canonical state. `/countyforge status` or a later authorized command performs stale reclamation inside the per-target state lane. Every canonical publication still requires a live lease, so a lapsed owner fails closed and completed evidence is preserved without an atomic comment primitive.
