...
State stores repository/target identity, current run and idempotency key, command/profile/head, attempt, workflow/check IDs, lifecycle state, lease, timestamps, disposition, and sanitized GitHub links. Rendering updates the existing comment rather than creating status spam. Every intake, execution, and maintenance writer uses one publication function that re-reads the bot-owned comment immediately before mutation and rejects the write unless its complete schema-validated state equals the writer's expected predecessor. The same publication mirrors the resulting lifecycle into an existing PR check. Terminal run data is immutable; display-only reconciliation may refresh links or timestamps without rewriting prior artifacts.
...
The canonical lease records owner workflow run/attempt, semantic key, command, target SHA, acquired/heartbeat/expiry timestamps, and a nonce. Complete-state compare-before-publication logic and the control lane produce one winner; a stale status, cancellation, claim, terminal publication, or maintenance write fails with `state_write_conflict` instead of replacing a newer state. Stage transitions update heartbeats. Check creation and check-ID publication failures convert an already-published queue to a retryable terminal failure. A queued run with no workflow owner or lease has a 30-minute preclaim deadline that status, a later command, or maintenance converts to `workflow_claim_timeout`. An authorized new command or maintenance run can reclaim an expired leased run by transitioning the old run to `stale`; completed evidence is never replaced. Maintenance only reconciles terminal recovery and never starts model work.
...
- **GitHub comment state is not a transactional database** -> Serialize control operations, validate marker ownership/schema, require compare-before-transition facts, and use leases plus idempotent reconciliation.
