...
- **D7** — `archive_timeout` is 300 seconds. This establishes the maximum idle WAL segment-switch interval while archiving is healthy, satisfying ADR-0002's "maximum five-minute WAL archive interval"; without it an idle database holds a partially filled segment indefinitely and the interval is unbounded. It is **not** a durable recovery-point guarantee: the real recovery point is the age of the latest WAL confirmed present in the archive, and that can exceed 300 seconds during any archive failure or backlog. The unimplemented observability requirement owns alerting on that age.
