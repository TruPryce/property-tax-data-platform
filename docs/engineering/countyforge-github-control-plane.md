...
- `retry` accepts failed, cancelled, timed-out, stale, or not-implemented runs; rejects active/successful work; requires an unchanged head; increments attempt; and preserves prior evidence. Planning retries restore the stored context fingerprint when present and retain the original identity format for legacy state without it. Implementation retries freshly resolve the merged planning approval, rerun eligibility, and require both the accepted-change hash and six-field approval fingerprint to match canonical state; legacy implementation state without that provenance is readable but must be replaced by a fresh `implement` command.
