...
- `retry` accepts failed, cancelled, timed-out, stale, or not-implemented runs; rejects active/successful work; requires an unchanged head; increments attempt; and preserves prior evidence. Initial planning intake and packet preparation fingerprint a newest-first window of at most 16 comments through the plan-command comment. Planning retries restore that stored context fingerprint and original trigger-comment cutoff, excluding the retry command, later discussion, and older comments outside the selected window while rejecting edits to issue evidence or selected original comments; legacy state without a fingerprint retains its original identity format. Implementation retries freshly resolve the merged planning approval, rerun eligibility, and require both the accepted-change hash and six-field approval fingerprint to match canonical state; legacy implementation state without that provenance is readable but must be replaced by a fresh `implement` command.
