...
| `countyforge-maintenance.yml` | Mark expired leases stale; never dispatch | None |
...
All three writing paths—intake/control, execution publication, and maintenance—use the same guarded publisher. It re-reads the trusted bot comment immediately before updating and requires its complete validated state to equal the expected predecessor; a competing transition produces `state_write_conflict`. After the comment update, the publisher mirrors that lifecycle into the existing PR check, so status reconciliation and stale recovery cannot leave an old check in progress. Interleaving tests cover status versus claim, status versus terminal publication, cancellation versus terminal publication, and maintenance versus a late heartbeat.

The canonical comment uses two concurrency facts: the transport ETag returned by GitHub and an application-level monotonic `revision`. New state starts at revision 1; every legal transition or other successful state mutation advances it, and a transition must use the current expected revision and produce the next revision exactly once. Canonical updates capture the exact comment ETag, send `If-Match`, map `412 Precondition Failed` to `state_write_conflict`, then perform at most one reread/rebase. A newer terminal state, retry attempt, cancellation, or heartbeat is never overwritten by stale publication. The ETag orders transport writes; the revision orders application state and appears in the hidden marker/history.

The lease records workflow run/attempt, semantic key, command, target SHA, acquisition/heartbeat/expiry timestamps, and a nonce. Its initial four-hour TTL exceeds the bounded execution workflow, and stage changes renew it. Claim and heartbeat require exact ownership. If the lease timestamp passes before terminal publication, the still-matching owner workflow/run/nonce may publish the terminal result; a replacement cannot, and a state already reclaimed as `stale` remains terminal. A queued state with no workflow owner or lease has a separate 30-minute preclaim deadline. Check creation/publication failures fail that queue immediately; status, a later command, or maintenance converts an abandoned queue to retryable `failed` state with `workflow_claim_timeout`. No recovery path overwrites completed evidence.
...
The repository-native `make prepr` gate does not activate an `issue_comment` workflow from a feature branch. After these workflows are present on the default branch, enable the command workflow in a controlled setting and post `/countyforge validate` on a same-repository controlled PR. Verify authorization, one canonical comment, dispatch, lease acquisition/release, no-secret preparation, `profile_not_implemented`, neutral check, sanitized artifacts, deduplication, status, and maintenance reconciliation. Then configure only the intended provider secret and post `/countyforge review`; verify that only the selected provider job receives it, target code is never executed in that job, packet/provenance binding passes, evidence is sanitized, and the check reaches the expected conclusion.
