...
6. Exercise `/countyforge status`, `/countyforge cancel` on an active synthetic run, `/countyforge retry` on a failed unchanged-head run, and scheduled/manual maintenance audit discovery.
...
The feature-branch test suite is the acceptance path before merge; this controlled default-branch run is the operational activation check. After an OpenSpec planning PR is merged by an authorized maintainer, a controlled `/countyforge implement <change>` smoke must additionally verify the isolated workspace, command policy, no-write model job, trusted reconstruction, and draft-only publication path.

## Canonical Status and Evidence

CountyForge edits one bot-owned status comment per issue or pull request. The primary visible table contains the current command, profile, abbreviated target SHA, the live default branch and its current SHA, retry eligibility, when that branch was last checked, lifecycle state, attempt, update time, and sanitized evidence link.

Retry eligibility answers, without a manual check, whether `/countyforge retry` would be accepted. It is `true` only when the run is retryable **and** the current default-branch SHA still equals the recorded target SHA. It reports `unknown` when the default branch cannot be resolved, or when the target is a pull request — a pull request is retried against its own head, so the default branch says nothing about it. These fields are display metadata: they are never stored in canonical state, never enter the hidden marker or semantic identity, and never authorize execution. `/countyforge retry` resolves the live target itself, so a stale rendering can mislead a reader but cannot permit a refused retry. When prior terminal runs exist, a bounded newest-first `Recent runs` table shows up to five archived command/profile/target/result/attempt/finished/evidence rows; it never shows `Pending` for history and uses a bounded disposition when evidence is unavailable. A hidden schema-valid marker contains bounded recovery state and the complete history facts.
...
Status requires authorization and performs no model call. It verifies canonical bot ownership plus the exact workflow repository, ID, event, path, and CountyForge run display identity; reconciles the workflow/check; and edits the existing comment. The workflow `name` field is display-oriented and advisory because the workflow configures a dynamic `run-name`. Canonical writes are serialized by the shared per-target `countyforge-state-*` job concurrency lane and reread-and-compare the persisted state, including its monotonic revision, immediately before a plain comment `PATCH`; a stale predecessor fails closed with `state_write_conflict` instead of overwriting newer state. The guarded publisher updates the existing PR check only after the canonical comment update succeeds. A queued run that has no workflow owner or lease after 30 minutes becomes retryable `failed` state. If no state exists, status returns a bounded no-run message.

The maintenance workflow runs twice per hour and can also be dispatched manually. It scans at most the newest 1,000 repository comments, fails closed if that documented bound is exhausted, and reports expired active leases or abandoned no-lease queues as bounded reconciliation candidates without writing canonical state. It also refreshes at most 16 canonical displays per sweep so retry eligibility does not sit stale between commands. Only settled retryable runs are refreshed: an active run's own lane rewrites its comment on every transition, and an out-of-lane write that lost that race would revert lifecycle, revision, disposition, and history. A settled comment updated in the last five minutes is skipped too, since that is most likely a new run claiming the target. A write happens only when the default branch, its SHA, or retry eligibility changed, or when the displayed observation is more than six hours old — otherwise the newest-updated listing would keep the same comments at its front and starve the rest. Each refresh re-renders from the state it just read and rereads and compares the comment before writing, so the hidden marker is byte-identical and lifecycle, history, revision, and idempotency identity are untouched. Apart from that display write, this repository-wide job remains audit-only because it cannot join every target's `countyforge-state-*` lane. An authorized `/countyforge status` or later command performs the candidate transition inside the target lane and updates the matching check. A malformed trusted-bot marker emits a bounded `invalid_state_detected` audit event and is skipped so it cannot suppress recovery for unrelated targets; same-target intake still fails closed on malformed state. The 1,000-comment bound is accepted for this repository's initial scale and must be revisited before broader/high-volume deployment. Maintenance never starts, retries, or mutates agent work.
...
| Planning reports `provider_generation_failed` | The provider returned the exact generation-failure sentinel before a valid structured document. Inspect sanitized stderr/events, confirm provider availability, and retry only on the unchanged head. |
...
| Lease expired | Let `/countyforge status` or a new authorized command perform the stale/claim-timeout transition inside the target state lane; scheduled maintenance is audit-only. Never edit the hidden marker manually. A run whose lease lapsed cannot be published by its original workflow (publication requires a live lease and fails closed), so recovery is always through stale reclamation plus `/countyforge retry`. |
| Command shows no effect (possible supersession) | Under a burst, an older pending state transaction on the shared `countyforge-state-*` lane may be superseded before it runs. Committed state is never corrupted; reissue `/countyforge status` or the control command. A superseded terminal publish is recovered by the next status or authorized command. |
