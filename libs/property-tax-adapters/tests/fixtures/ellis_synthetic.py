...
VALID_ODS_PACKAGE = (
    ...
    + b"\x00" * 4  # crc
    + len(_ODS_MEDIA_TYPE).to_bytes(4, "little")  # compressed size
    + len(_ODS_MEDIA_TYPE).to_bytes(4, "little")  # uncompressed size
...
#: A header declaring zero bytes while carrying the media type. Structurally a
#: ZIP local header, but it describes a member that is not there.
ZERO_DECLARED_SIZE = (
    _ZIP_LOCAL_HEADER
    + b"\x14\x00\x00\x00\x00\x00"
    + b"\x00" * 12
...
#: Declared sizes that disagree with one another, which a stored member cannot
#: have.
MISMATCHED_DECLARED_SIZES = (
    _ZIP_LOCAL_HEADER
    + b"\x14\x00"
    + b"\x00\x00"
    + b"\x00\x00"
    + b"\x00" * 4
    + b"\x00" * 4
    + len(_ODS_MEDIA_TYPE).to_bytes(4, "little")
    + (len(_ODS_MEDIA_TYPE) + 1).to_bytes(4, "little")
    + b"\x08\x00"
    + b"\x00\x00"
    + b"mimetype"
    + _ODS_MEDIA_TYPE
