...
class PacsLayout:
    ...
    def __init__(self, layout_id: str, layout_version: str, fields: tuple[PacsField, ...]) -> None:
        ...
        self._fields = fields
        # Assigned last: `__setattr__` seals the instance once this exists.
        self._fingerprint = _fingerprint(layout_id, layout_version, fields)

    def __setattr__(self, name: str, value: object) -> None:
        """Seal the instance once the fingerprint has been computed.

        Read-only properties are not immutability: the private attributes
        behind them stayed assignable, so setting `_layout_version` produced a
        layout that validated while reporting the approved digest beside a
        version it no longer described.
        """

        if hasattr(self, "_fingerprint"):
            raise AttributeError(f"PacsLayout is immutable once fingerprinted; cannot set {name!r}")
        object.__setattr__(self, name, value)

    def __delattr__(self, name: str) -> None:
        raise AttributeError(f"PacsLayout is immutable; cannot delete {name!r}")
