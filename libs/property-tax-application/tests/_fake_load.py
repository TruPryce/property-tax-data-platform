...
this implements: `S0`-`S6`, `G1`, `W1`-`W7`, `W9`-`W12`, `C1`-`C2`, `A1`, and the
derived invariant `I1`.  It exists so the
...
`I1` — every live handle belongs to the open account — is deliberately *not*
asserted in here.  A subject that checks its own invariant proves nothing: the
check and the behaviour would fail together and the suite would never know. The
suite asserts it from outside, after every operation, which is the only place
the assertion can fail independently of what it is checking.
"""
...
def run(value: str = "run-1") -> ProcessingRunRef:
    """A run reference, the concrete owned type rather than a stand-in."""

    return ProcessingRunRef(value)


def outcome(*, accepted: bool = True) -> ReleaseProcessingOutcome:
    """An outcome whose disposition selects a completion branch.

    Minimally valid in each direction, because the value enforces its own
    evidence seal: a rejected release has produced at least one diagnostic.
    """

    if accepted:
        return ReleaseProcessingOutcome(ReleaseDisposition.ACCEPTED)
    return ReleaseProcessingOutcome(
        ReleaseDisposition.REJECTED,
        diagnostics=(ReleaseDiagnosticRecord("record_rejected"),),
...
class FakeStore:
    ...
    def __init__(self) -> None:
        self.loads: dict[tuple[ReleaseIdentity, ProcessingRunRef], tuple[object, ...]] = {}
        self.outcomes: dict[tuple[ReleaseIdentity, ProcessingRunRef], ReleaseProcessingOutcome] = {}
        #: Which pairings have completed, recorded independently of whether the
        #: completion persisted any record.  Judging this from `loads` would make
        #: a rejected pairing look incomplete forever: the rejected branch
        #: persists no row, so each retry would rewrite the recorded outcome, and
        #: a retry carrying an accepted disposition would manufacture a canonical
        #: load for a run whose durable outcome says it was rejected.
...
class FakeRepository:
    ...
    def open_load(
        ...
        # `S0` is complete or the session does not open, and complete means the
        # owned types rather than whatever a caller had to hand.  A port that
        # took a raw run id would let a caller name a run that was never
        # recorded, which is the one thing `ProcessingRunRef` exists to prevent,
        # and checking `is None` does not notice that at all.
        if not isinstance(release, ReleaseIdentity):
            raise LoadRefused("S0", "release")
        if not isinstance(run, ProcessingRunRef):
            raise LoadRefused("S0", f"run is a {type(run).__name__}")
        if not isinstance(outcome, ReleaseProcessingOutcome):
            raise LoadRefused("S0", f"outcome is a {type(outcome).__name__}")
...
class FakeSession:
    ...
    def write(self, batch: CanonicalRecordBatch) -> None:
        ...
        # There is no `W8`.  Account scope is `I1`, a derived invariant: `W9`
        # confines a retained handle to the continuing account, the success
        # transition releases the rest at completion, and `W7` confines a
        # release to what is live — so `S4` only ever holds the open account's
        # handles and a delta has no other account's handle to name.  It is
        # asserted below rather than checked here, because a check with no
        # reachable counterexample is a rule an implementation cannot honour.
