...
are intrinsic to a batch and enforced here, in `CanonicalRecordBatch`; `W1`-`W7`,
`W9`-`W12`, `C1`-`C2`, `A1` and the general rule `G1` need session history and
belong to an implementation of `ReleaseLoadSession`.  There is no `W8`: account
scope is `I1`, a derived invariant an implementation holds rather than a
precondition it checks, and the identifier is retired rather than reused.
...
`ReleaseProcessingOutcome` belong to the `processing-run-values` capability and
are imported from `runs.py`.  An earlier draft declared local structural
protocols for them instead, and the review measured what that costs — an empty
runtime-checkable protocol returns `True` for `isinstance(None, …)`,
`isinstance(7, …)` and `isinstance("raw-run-id", …)`, so the substitute accepted
exactly the raw persistence value the reference type exists to keep out.
"""
...
def account_of(record: CanonicalRecord) -> AccountIdentity:
    ...
    its deltas.  `I1` turns on that: the invariant that every live handle belongs
    to the open account only holds if a rule permitting a delta never takes the
    delta as its own evidence.  Every record reaches its account by the parent it
    already holds, so no correlation value is consulted and no new key is
    invented.
    """
...
class AccountSnapshotRef:
    ...
    def __post_init__(self) -> None:
        # The same rule `ProcessingRunRef` is held to, borrowed from the
        # capability that owns it rather than restated in a weaker form. Opacity
        # is not permission to carry anything: an implementation indexes
        # candidates by these locators and a caller may put one in a set, and a
        # payload free to change is one that moves underneath both.
...
class CanonicalRecordBatch:
    ...
    def carries(self, account: AccountIdentity) -> bool:
        ...
        account's handles in a delta does not carry it, which is what keeps a
        delta from being the evidence for that same delta.
        """
...
class ReleaseLoadCompletion:
    ...
    def __post_init__(self) -> None:
        # An annotation nothing checks is a comment. A completion carrying a raw
        # run id would hand a caller back exactly the value the reference type
        # exists to keep out of every port it crosses.
        if not isinstance(self.run, ProcessingRunRef):
            raise ValueError(f"run must be a ProcessingRunRef, got {type(self.run).__name__}")
...
class ReleaseLoadSession(Protocol):
    ...
    def write(self, batch: CanonicalRecordBatch) -> None:
        ...
        Enforces `W1`-`W7` and `W9`-`W12` against session history, and holds
        `I1` afterwards.  Nothing becomes durable here whatever is accepted.
        """
